...
# Default RECIPE 'pbenv' can be overridden by 'RECIPE=foo setup.sh'
RECIPE=${RECIPE:-pbenv}
# Default ENV_NAME 'pbenv' can be overridden by 'ENV_NAME=foo setup.sh'
...
if [[ -d ~/.conda/envs/${RECIPE}  ]] && [[ "${FORCE}" != "1" ]]
then
    echo Environment ${RECIPE} exists.  Use FORCE=1 to remove it anyway.
    exit 1
fi
...
RECIPE_HOME=$(conda info --envs | grep ${RECIPE} | awk 'END{print $NF}')
mkdir -p ${RECIPE_HOME}/etc/conda/activate.d
echo "export CUDA_HOME=/usr/local/cuda-10.2" \
    > ${RECIPE_HOME}/etc/conda/activate.d/CUDA_HOME.sh
