...
Hook hooks a layer to allow its output to be modified.
Trace retains the input, output, or output gradients for a layer.

InstrumentedModel is an older API which is for hooking
many layers at once.
'''
...
class Hook(AbstractContextManager):
    '''
    A context manager to handle a pytorch forward hook.

        with Hook(net, 'layer.name') as hook:
            hook.fn = lambda output: output.clamp(0, 1)
            y = net(x)

    or:

        def make_residual(input, output):
            return output + input[0]
        with Hook(net, 'layer.name', fn=make_residual)
            y = net(x)

    The hook is automatically removed at the end of the `with` context.
    The callback function can be provided as the last `fn` argument of
    the Hook constructor, or set as in the example above.  The argument
    names for `fn` are important: the function should be declared with
    named keyword arguments namd `module`, `input`, `output` (or any
    subset as needed for the function), and it should return the
    replacement output.
    '''

    def __init__(self, module, layer=None, fn=None):
        ...
        def callback(module, input, output):
            if self.fn is not None:
                output = invoke_with_optional_args(
                        fn, module=module, input=input, output=output)
                return output
        self.registered_hook = module.register_forward_hook(callback)
    ...
    def __exit__(self, type, value, traceback):
        self.close()
        if type is not None and issubclass(type, StopForward):
            return True
...
class Trace(Hook):
    ...
    the output object is returned, but options can control this:
        ...
        stop=True - interrupt execution of the network immediately
            after this layer.  Useful for stopping a network short
            to avoid expensive work after obtaining needed data.
            Warning: raises a StopForward exception that skips to
            the end of the enclosing `with`.
    '''
    ...
    def __init__(self, module, layer=None, retain_output=True,
        ...
        '''
        retainer = self
        def retain_hook(module, input, output):
            if retain_input:
            ...
            if stop:
                raise StopForward()
            return output
        super().__init__(module, layer=layer, fn=retain_hook)
...
class InstrumentedModel(torch.nn.Module):
    ...
    def retain_layer(self, layername, detach=None,
                     retain_grad=None, clone=None):
        '''
    ...
    def retain_layers(self, layernames, detach=None,
                      retain_grad=None, clone=None):
        '''
    ...
    def _hook_sequential(self):
        ...
        def new_forward(this, x, layer=None, first_layer=None,
                        last_layer=None):
