...
class SeqStyleGAN2(nn.Sequential):
    ...
    def __init__(
        ...
        for i in range(n_mlp):
            style_layers.append(EqualLinearLSeq(
...
class EqualLinearLSeq(nn.Sequential):
    ...
    def __init__(self, in_dim, out_dim, bias=True, bias_init=0,
        ...
        super().__init__(OrderedDict([
            ('scaled_linear', self.LatentLayer(lambda x:
                F.linear(x, self.weight * self.scale))),
            ('bias_lrelu', self.LatentLayer(lambda x:
                op.fused_leaky_relu(x, self.bias * self.lr_mul)))
        ]))
        self.bias_init = bias_init
        self.lr_mul = lr_mul
        self.scale = (1 / math.sqrt(in_dim)) * lr_mul
...
class NoiseInjectionF(nn.Module):
    ...
    def forward(self, data):
        ...
        if noise is None:
            noise = np.random.RandomState().randn(
...
class PickLatent(nn.Module):
    ...
    def forward(self, d):
        ...
        return DataBag(d, style=d.latent[:,self.index],
                          noise=d.get(f'noise_{self.index}', None))
